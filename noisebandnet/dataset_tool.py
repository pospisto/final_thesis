...
def load_control_params(path, device, repeats):
    ...
    '''
    control_params = []  # This will hold lists of tensors
    last_name = None  # To track the previous file prefix
    num_of_features = None

    for idx, file in enumerate(sorted(os.listdir(path))):  # Sorting ensures correct order
        if file.endswith(".npy"):
            # Extract prefix (before "_feature.npy")
            current_name = file.split('_')[0]

            feature_tensor = torch.from_numpy(np.load(os.path.join(path, file))).repeat(1, repeats).to(device)

            if last_name is None or current_name == last_name:
                control_params.append(feature_tensor)  # Initialize for first three features
                last_name = current_name
                num_of_features = idx
            else:
                control_params[idx % num_of_features] = torch.hstack((control_params[idx % 3], feature_tensor))
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if new_audio_index < 0:
            x_audio = torch.cat([self.audio_data[..., new_audio_index:],
                                 self.audio_data[..., :new_audio_index + self.audio_size_samples]], dim=-1)
            for i in range(len(self.control_params)):
                x_control_params.append(torch.cat([self.control_params[i][..., new_audio_index:],
                                                   self.control_params[i][...,
                                                   :new_audio_index + self.audio_size_samples]], dim=-1))
        # wrap around
        elif new_audio_index + self.audio_size_samples > self.audio_data.shape[-1]:
            x_audio = torch.cat([self.audio_data[..., new_audio_index:], self.audio_data[...,
                                                                         :self.audio_size_samples - (
                                                                                     self.audio_data.shape[
                                                                                         -1] - new_audio_index)]],
                                dim=-1)
            for i in range(len(self.control_params)):
                x_control_params.append(torch.cat([self.control_params[i][..., new_audio_index:],
                                                   self.control_params[i][..., :self.audio_size_samples - (
                                                               self.audio_data.shape[-1] - new_audio_index)]], dim=-1))
        # normal slicing
        else:
            x_audio = self.audio_data[..., new_audio_index:new_audio_index + self.audio_size_samples]
            for i in range(len(self.control_params)):
                x_control_params.append(
                    self.control_params[i][..., new_audio_index:new_audio_index + self.audio_size_samples])
    ...
    def _get_single_item(self, idx):
        ...
        # normal slicing
            ...
            # 1. Debugging and length checks for x_audio
        print(f"Délka x_audio: {x_audio.shape}")
        if x_audio.shape[-1] != self.audio_size_samples:
            print(
                f"Varování: Délka x_audio ({x_audio.shape[-1]}) se neshoduje s audio_size_samples ({self.audio_size_samples})")

            # 2. Debugging and length checks for x_control_params
        print(f"Délky x_control_params:")
        for i, param in enumerate(x_control_params):
            print(f"  Param {i}: {param.shape}")
            if param.shape[-1] != self.audio_size_samples:
                print(
                    f"Varování: Délka x_control_params[{i}] ({param.shape[-1]}) se neshoduje s audio_size_samples ({self.audio_size_samples})")
        return x_audio, x_control_params
